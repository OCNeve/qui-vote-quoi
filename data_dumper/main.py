...
import csv
import pandas as pd
from data_dumper.file_manager import FileManager




def extract_names_data():
	fm = FileManager(url='https://www.insee.fr/fr/statistiques/fichier/2540004/dpt2021_csv.zip', name='./names.csv.zip', path = './data_dumper/data')
	fm.download_and_extract()
	data = []
	with open('./data_dumper/data/dpt2021.csv', mode='r', encoding='utf-8') as file:
...
def extract_elections_data():
	fm = FileManager(url='https://www.data.gouv.fr/fr/datasets/r/98eb9dab-f328-4dee-ac08-ac17211357a8', name='./elections.xlsx', path = './data_dumper/data')
	fm.download_file()
	return pd.read_excel('./data_dumper/data/elections.xlsx')

def import_names_data(pgc, data):
...
def import_election_data(pgc, df):
...
def _import():
	...
	import_names_data(pgc, data)

	df = extract_elections_data()
	import_election_data(pgc, df)
